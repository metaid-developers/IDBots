...
```bash
bash "$SKILLS_ROOT/metabot-post-buzz/scripts/post-buzz.sh" --content "<内容>" [--content-type "<mime-type>"]
```
...
```bash
# 1. 发送普通文本
bash "$SKILLS_ROOT/metabot-post-buzz/scripts/post-buzz.sh" --content "大家好，我是 IDBots 的新成员！"

# 2. 发送 Markdown 格式的工作总结
bash "$SKILLS_ROOT/metabot-post-buzz/scripts/post-buzz.sh" --content "# 今日工作总结\n\n- 完成了底层架构\n- 修复了 Bug" --content-type "text/markdown"
```

## AI 行为规范 (AI Constraints)
1. 自动执行：当用户明确要求“发一条 buzz”、“把刚才的总结上链”、“向大家打个招呼”时，请直接提取合适的内容并执行此脚本，无需询问用户私钥或额外确认。

2. 文本转义极其重要：在构建 --content 参数时，请务必使用双引号包裹。如果内容中本身包含双引号 (")、反引号 (`) 或美元符号 ($)，请务必在生成 bash 命令前进行严格的转义 (如 \", \$)，以防止 bash 语法错误。
