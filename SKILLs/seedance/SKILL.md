...
IDBots 会自动读取系统环境变量，确保在启动 IDBots 前已设置 `ARK_API_KEY`。
...
IDBots 已包含 Node.js 运行时，所有必要的依赖已被自动打包。Windows 和 Mac 用户无需额外配置。
...
**路径说明**：下面的示例使用 `$SKILLS_ROOT` 环境变量来引用脚本路径。IDBots 会自动设置这个变量，指向实际的 SKILLs 目录位置，因此无需手动修改路径。
...
或在 IDBots 对话中说："用 Seedance 2.0 生成视频..."
