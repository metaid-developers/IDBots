import React from 'react';
import MetaAppsManager from './MetaAppsManager';
import SidebarToggleIcon from '../icons/SidebarToggleIcon';
import ComposeIcon from '../icons/ComposeIcon';
import WindowTitleBar from '../window/WindowTitleBar';
import type { MetaAppRecord } from '../../types/metaApp';
import { getMetaAppsHeaderModel } from './metaAppViewModel.js';
import { i18nService } from '../../services/i18n';

interface MetaAppsViewProps {
  isSidebarCollapsed?: boolean;
  onToggleSidebar?: () => void;
  onNewChat?: () => void;
  onStartTaskWithMetaApp?: (app: MetaAppRecord) => Promise<void> | void;
  updateBadge?: React.ReactNode;
}

const MetaAppsView: React.FC<MetaAppsViewProps> = ({
  isSidebarCollapsed,
  onToggleSidebar,
  onNewChat,
  onStartTaskWithMetaApp,
  updateBadge,
}) => {
  const isMac = window.electron.platform === 'darwin';
  const header = getMetaAppsHeaderModel((key) => i18nService.t(key));

  return (
    <div className="flex-1 flex flex-col dark:bg-claude-darkBg bg-claude-bg h-full">
      <div className="draggable flex h-12 items-center justify-between px-4 border-b dark:border-claude-darkBorder border-claude-border shrink-0">
        <div className="flex items-center space-x-3 h-8">
          {isSidebarCollapsed ? (
            <div className={`non-draggable flex items-center gap-1 ${isMac ? 'pl-[68px]' : ''}`}>
              <button
                type="button"
                onClick={onToggleSidebar}
                className="h-8 w-8 inline-flex items-center justify-center rounded-lg dark:text-claude-darkTextSecondary text-claude-textSecondary hover:bg-claude-surfaceHover dark:hover:bg-claude-darkSurfaceHover transition-colors"
              >
                <SidebarToggleIcon className="h-4 w-4" isCollapsed={true} />
              </button>
              <button
                type="button"
                onClick={onNewChat}
                className="h-8 w-8 inline-flex items-center justify-center rounded-lg dark:text-claude-darkTextSecondary text-claude-textSecondary hover:bg-claude-surfaceHover dark:hover:bg-claude-darkSurfaceHover transition-colors"
              >
                <ComposeIcon className="h-4 w-4" />
              </button>
              {updateBadge}
            </div>
          ) : null}
          <h1 className="text-lg font-semibold dark:text-claude-darkText text-claude-text">
            {header.title}
          </h1>
        </div>
        <WindowTitleBar inline />
      </div>

      <div className="flex-1 overflow-y-auto min-h-0">
        <div className="max-w-3xl mx-auto px-4 py-6">
          <MetaAppsManager onStartTaskWithMetaApp={onStartTaskWithMetaApp} />
        </div>
      </div>
    </div>
  );
};

export default MetaAppsView;
