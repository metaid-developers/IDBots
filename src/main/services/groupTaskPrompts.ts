...
function chairPlaybookRules(language: AppLanguage): string[] {
  ...
  return [
    ...
    '- VERIFICATION ECONOMY: the host already runs the DETERMINISTIC checks on every [DELIVERABLE] (on-chain pin existence across multiple sources, hash/size where applicable, public reachability) and rides the results into your context as verification facts; worker evidence discipline supplies the raw checksums. Do NOT re-download and re-hash what a host verification fact or a worker-supplied checksum already confirms — spend your gate on the SEMANTIC layer: does the content match the frozen口径 and acceptance criteria, is it complete, plausible, and honestly reported. Re-run a deterministic check ONLY when evidence is missing or two sources contradict each other.',
