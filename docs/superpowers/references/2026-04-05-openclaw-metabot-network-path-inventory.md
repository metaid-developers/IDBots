...
| `payment_txid` | Paid path uses `paymentTxid` in `src/main/main.ts:2264` | Primary paid-order correlation key used to bind buyer/seller order rows and later requester/provider transport envelopes | Yes |
| `order_reference_id` | Free path falls back to `orderReference` in `src/main/main.ts:2265` | Compatibility correlation key for free orders when there is no real payment txid | Yes |
...
| `payment_txid` | Parsed from ORDER plaintext in `src/main/services/privateChatDaemon.ts:797-819` | Which paid-order reference unlocks execution and later transport correlation | Yes |
| `order_reference_id` | Parsed from ORDER plaintext in `src/main/services/privateChatDaemon.ts:797-819` | Which free-order compatibility reference unlocks execution when there is no real txid | Yes |
...
| `payment_txid` | Delivery payload currently serializes `paymentTxid: orderTrackingId` in `src/main/services/privateChatDaemon.ts:984-989`; buyer-side matching uses the same correlation key in `src/main/services/privateChatDaemon.ts:1083-1093` | The paid order being completed and the minimum cross-layer delivery correlation field | Yes |
| `order_reference_id` | Gap: current delivery payload reuses the legacy `paymentTxid` slot for free-order compatibility instead of carrying a first-class field | Compatibility correlation field V1 should preserve alongside `payment_txid` for free orders | Yes |
...
| `payment_txid` | Current buyer-side link field in `src/main/services/privateChatDaemon.ts:1083-1093` | Minimum compatibility join key for delivered buyer orders and requester-side reinjection | Yes |
| `order_reference_id` | Gap: current visible delivery does not carry a first-class free-order correlation field separate from `payment_txid` | V1 requester bridge should preserve it for free-order compatibility | Yes |
