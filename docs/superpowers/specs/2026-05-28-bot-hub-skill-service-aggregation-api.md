...
提供服务详情页首屏和下单弹窗所需的服务详情、provider 信息、支付上下文、可下单状态、评分摘要和最小链上证明。详情页不应再额外请求 provider profile、chat pubkey、rating aggregate 或图标资产。
...
- `chainProof` 只返回最小链上证明字段，供前端做“查看链上来源”或调试展示；v1 详情主响应不返回完整历史 revisions。
...
| M6 | HTTP detail endpoint；ordering readiness；chainProof 最小证明；latest ratings | 集成测试覆盖 currentPinId/sourceServicePinId 查询、不可订购原因、MRC20 详情 |
| M7 | 第二阶段扩展：ratings 分页、revisions 分页、stats、refund risk | 集成测试覆盖分页与指标 |

第一阶段建议交付到 M6。M7 可在 Bot Hub 首屏和详情页切换到聚合接口后继续增强。`relatedServices` 暂不进入 milestone，除非后续产品界面明确需要推荐区。
