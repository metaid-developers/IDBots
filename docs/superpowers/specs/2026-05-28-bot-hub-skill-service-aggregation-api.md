...
- BTC/DOGE/OPCAT 如已有 indexer 实现，后续可逐步接入，但 v1 不要求跨链版本链折叠。
...
- 聚合端内部服务主键建议使用 `${chainName}:${sourceServicePinId}`，避免未来跨链 pin id 碰撞或错误折叠。
...
| `chainName` | string | 否 | - | `mvc`、`btc`、`doge`、`opcat`；v1 至少支持 `mvc` |
...
- 按 `sourceServicePinId` 折叠版本链：
  ...
  - 同一服务只返回当前有效版本。
  - 版本链折叠不跨链；modify/revoke 的 `chainName` 必须与 create 相同，否则视为另一条独立服务记录或无效变更。
- 默认可见规则：
...
| `chainName` | string | 否 | - | 可选链名，用于跨链部署后消除歧义 |
...
- modify/revoke 不允许跨链引用 create；如果 `originalId` 指向另一条链上的 pin，聚合端不得折叠到同一服务。
...
| Milestone | 内容 | 验收 |
| --- | --- | --- |
| M1 | `meta-socket` wire MVC indexer；索引 `/protocols/skill-service`；按 `originalId` 折叠 create/modify/revoke；PebbleDB 持久化当前服务视图 | 构造 create+modify+revoke pin，验证聚合视图只保留当前有效服务，revoke 后默认不可见 |
