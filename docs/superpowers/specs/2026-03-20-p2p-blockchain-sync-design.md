...
**本阶段范围说明**：MRC20 代币协议解析、资产聚合索引、跨链 Teleport 验证均推迟到后续阶段实现。MRC20 本质也是 PIN，P2P 层会正常同步其原始 PIN 数据，但不做资产层面的解析和余额计算。MAN 中 MRC20 相关代码在融合时保留但不启用。

### 3.2 P2P 数据流

**PIN 的合法性由区块链保证，P2P 层只负责传播已经过区块链验证的数据。**

- PIN 来自已确认区块：`confirmed=true`，100% 有效，不可逆
- PIN 来自矿工内存池：`confirmed=false`，99.9% 有效（极少数情况因费率不足被丢弃）
- P2P 节点传播的 PIN 均来自区块链网络（已确认块或内存池），无需额外链上验证

**上行（本地新 PIN → 广播）：**
```
用户构建交易 → 广播到区块链节点（UTXO 有效性由链保证）
  → 交易进入矿工内存池（ZMQ 实时通知）
  → man-p2p 扫描到新 PIN，写入 PebbleDB（confirmed=false）
  → GossipSub 广播 PIN 元数据
    （PIN ID + 路径 + 发布者地址 + confirmed 状态，不含内容）
  → 订阅了该地址的节点收到通知，按需拉取完整 PIN 内容
  → 交易被打包进块后，更新 confirmed=true
```
...
### 6.1 PIN confirmed 状态

PIN 的合法性由区块链网络保证，man-p2p 不做额外链上验证。只需追踪 `confirmed` 状态：

| 状态 | 来源 | 含义 |
|------|------|------|
| `confirmed=true` | 已打包进区块 | 100% 有效，不可逆 |
| `confirmed=false` | 矿工内存池 | 99.9% 有效，极少数因费率不足 14 天后被丢弃 |

MAN 已实现 confirmed 状态追踪（ZMQ mempool + 区块扫描），man-p2p 直接复用。

**`p2p_max_content_size_kb` 字段说明**：硬性截断阈值。超过此大小时只同步 PIN 元数据（PIN ID、路径、发布者地址、txid、confirmed 状态），不拉取内容字节。内容字段在 PebbleDB 中存为空，标记 `content_fetched=false`。应用层查询时若需要内容，再按需拉取。
...
1. 以 MAN 为基础，移植 meta-file-system 的用户信息索引和文件内容检索（MRC20 资产解析代码保留但不启用）
...
| 恶意节点传播伪造 PIN | 低 | 低 | PIN 载体是区块链 tx，链本身保证合法性；P2P 层只传播来自链的数据 |
